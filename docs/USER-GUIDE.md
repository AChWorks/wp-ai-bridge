...
Enable the smallest access set needed for the current work. v0.4.0 registers only the canonical `wp-ai-bridge...` admin pages; former WP Native Builder bookmarks are retired after the identity cutover.
...
The plugin uses standard WordPress localization APIs. Persian (`fa_IR`) is bundled, and the admin screens are designed to work in both RTL and LTR WordPress installations. The text domain is the canonical `wp-ai-bridge` identity.
