...
Create cleanup never trusts a filterable REST response UUID, link, or `Location` header. For one exact internal create request, the Bridge adds an ephemeral correlation only at the end of Core's `rest_pre_insert_application_password` preparation and observes `wp_create_application_password` immediately after Core persistence, before additional REST fields can fail. That early event supplies the exact persisted UUID; the correlation is not stored in the credential. The later exact-request `rest_after_insert_application_password` event independently confirms the same UUID and the Core-formatted one-time password before any successful response is accepted. Missing/substituted output, a later REST error, or an exception cleans up only the unambiguous early-captured UUID. Multiple matching preparations/creates, or any state where exact identity cannot be proven safely, returns `application_password_create_recovery_required` without guessing a destructive target; inspect that user's Application Passwords before retrying.
