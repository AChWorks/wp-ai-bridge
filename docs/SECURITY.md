...
Create cleanup never trusts a filterable REST response UUID, link, `Location` header, caller value, name, app ID, list order, or public Application Password action as destructive authority. Before inspecting credential storage, Bridge first runs Core's exact target-specific create permission check; this prevents Core's legacy UUID-backfill behavior in the Application Password reader from mutating storage for an unauthorized caller. Only after that authorization succeeds does Bridge snapshot the target user's Core Application Password state. For the exact internal REST create request it then observes Core's `_application_passwords` user-metadata write before persistence. A write qualifies only when the nearest `WP_Application_Passwords::create_new_application_password()` call is directly owned by `WP_REST_Application_Passwords_Controller::create_item()` for that exact `WP_REST_Request`, and the proposed credential state is exactly the current state plus one new UUID. Bridge retains only in-memory UUID, stored-hash and full-item fingerprints as private provenance; it does not add a correlation token to public Core action arguments and does not emit or log those fingerprints. The later exact-request `rest_after_insert_application_password` event is used only to cross-check the same UUID and Core-formatted one-time password before success. Before cleanup Bridge re-reads Core state and revokes through the fixed Core REST DELETE route only when the captured UUID still has the exact captured fingerprint. Successful create additionally requires final stored state to equal the pre-request baseline plus exactly that credential. Nested/reentrant/provider changes that leave extra or altered credential state therefore force `application_password_create_recovery_required` after any provably exact outer cleanup; they never turn a nested or replayed credential into cleanup authority. This remains true when a pre-existing global `all` hook or same-priority `wp_create_application_password` callback nests another credential and throws before later action callbacks execute.
