...
Create recovery does not use a Bridge correlation token on `wp_create_application_password` and never treats a public action callback as cleanup provenance. For the exact internal Core REST create request, Bridge privately snapshots the target user's Application Password state and observes Core's `_application_passwords` user-metadata write before persistence. A write qualifies only when the nearest `WP_Application_Passwords::create_new_application_password()` call is directly owned by `WP_REST_Application_Passwords_Controller::create_item()` for that exact request, and the proposed state is exactly the current state plus one new credential. Bridge captures that proposed credential's UUID, stored-hash fingerprint and full item fingerprint only as in-memory provenance; none is returned or logged. The later exact-request `rest_after_insert_application_password` event is a success cross-check for the same UUID and Core-formatted one-time password, not cleanup authority. Before any cleanup DELETE, Bridge re-reads Core state and requires the captured UUID to retain the exact captured fingerprint; mutation still goes through the fixed Core REST DELETE route. A successful create additionally requires final stored state to equal the pre-request baseline plus exactly that credential. If nested/reentrant/provider behavior leaves any extra or changed credential state, or exact provenance cannot be proven, Bridge returns `application_password_create_recovery_required` instead of guessing a destructive target. This remains true even when a pre-existing `all` hook or same-priority `wp_create_application_password` callback nests another credential and throws before later action callbacks run.
