...
Create recovery is bound before response preparation and before error-capable additional REST-field updates. The Bridge stamps one exact internal request with an ephemeral correlation at the end of `rest_pre_insert_application_password`, captures the persisted UUID from Core's earlier `wp_create_application_password` event, and then cross-checks that UUID plus the Core-formatted one-time password at the exact-request `rest_after_insert_application_password` event. The final response must preserve both values exactly. A later REST error, exception, missing/substituted credential output, or malformed response can revoke only that unambiguous early-captured UUID. Reentrant/multiple matching creates are treated as ambiguous and never become cleanup authority. If exact created identity cannot be proven safely, the Bridge returns `application_password_create_recovery_required` without guessing a DELETE target; inspect that user's Application Passwords before retrying.
