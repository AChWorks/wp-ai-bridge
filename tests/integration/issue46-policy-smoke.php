...
function wpai_issue46_policy_assert( $condition, $message ) {
	if ( ! $condition ) {
...
try {
	...
	wpai_issue46_policy_assert( $read instanceof WP_Ability, 'Source reading Ability missing.' );
	$result = $read->execute( array( 'action' => 'list', 'kind' => 'plugin' ) );
	wpai_issue46_policy_assert( is_wp_error( $result ), 'Deployment policy constant did not deny source editing.' );
