...
function wpnb_issue54_replay_assert( $condition, $message ) {
	if ( ! $condition ) {
		throw new RuntimeException( $message );
	}
}

function wpnb_issue54_replay_b64( $value ) {
	return rtrim( strtr( base64_encode( $value ), '+/', '-_' ), '=' );
}

wpnb_issue54_replay_assert( function_exists( 'openssl_pkey_new' ), 'OpenSSL is required for client-assertion replay coverage.' );
...
wpnb_issue54_replay_assert( false !== $private_key, 'Could not generate the replay-window RSA fixture.' );
$key_details = openssl_pkey_get_details( $private_key );
wpnb_issue54_replay_assert( is_array( $key_details ) && ! empty( $key_details['rsa']['n'] ) && ! empty( $key_details['rsa']['e'] ), 'Replay-window RSA fixture details are unavailable.' );
...
$jwk = array(
	...
	'use' => 'sig',
	'alg' => 'RS256',
	'n'   => wpnb_issue54_replay_b64( $key_details['rsa']['n'] ),
	'e'   => wpnb_issue54_replay_b64( $key_details['rsa']['e'] ),
);
...
$http_mock = static function ( $preempt, $args, $url ) use ( $jwks_uri, $jwk ) {
	...
	wpnb_issue54_replay_assert( 0 === (int) ( $args['redirection'] ?? -1 ), 'Replay-window JWKS retrieval allowed redirects.' );
	wpnb_issue54_replay_assert( (int) ( $args['timeout'] ?? 0 ) <= 8, 'Replay-window JWKS retrieval exceeded the timeout bound.' );
	wpnb_issue54_replay_assert( 65537 === (int) ( $args['limit_response_size'] ?? 0 ), 'Replay-window JWKS retrieval lost the response-size bound.' );
	return array(
...
$input = wpnb_issue54_replay_b64( wp_json_encode( $header ) ) . '.' . wpnb_issue54_replay_b64( wp_json_encode( $claims ) );
$signature = '';
wpnb_issue54_replay_assert( openssl_sign( $input, $signature, $private_key, OPENSSL_ALGO_SHA256 ), 'Could not sign the replay-window client assertion.' );
$assertion = $input . '.' . wpnb_issue54_replay_b64( $signature );
...
wpnb_issue54_replay_assert( true === $result, 'Maximum-lifetime additional-client assertion was rejected.' );

$replay_key    = 'wpnb_oauth_assertion_' . substr( hash( 'sha256', $client_id . "\0" . $jti ), 0, 40 );
$replay_expiry = (int) get_option( $replay_key, 0 );
wpnb_issue54_replay_assert( $replay_expiry > 0, 'Replay-window marker was not persisted.' );
wpnb_issue54_replay_assert(
