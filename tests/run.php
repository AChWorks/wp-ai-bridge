...
$gf_created = call_user_func(
	$gf_upsert['execute_callback'],
	array(
		'action' => 'create',
		'form'   => array(
			'title'       => 'Provider contract fixture',
			'description' => 'Fast GFAPI fallback coverage',
			'fields'      => array(),
		),
	)
);
...
$gf_activated = call_user_func(
	$gf_status['execute_callback'],
	array(
		'id'     => 1,
		'active' => true,
	)
...
foreach ( array( 'wp-native-builder', 'wp-native-builder-documents', 'wp-native-builder-tasks', 'wp-native-builder-activity', 'wp-native-builder-settings' ) as $legacy_slug ) {
	wpai_assert( ! isset( $GLOBALS['wpai_test']['submenu_pages'][ null ][ $legacy_slug ] ), 'Retired WP Native Builder admin alias is not registered: ' . $legacy_slug );
}
ob_start();
