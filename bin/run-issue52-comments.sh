...
export COMPOSE_PROJECT_NAME="wpnb-issue52-${safe_tag}-$$"
...
"${wp[@]}" core install --url=https://localhost --title='WP AI Bridge Issue 52' --admin_user=admin --admin_password='integration-only-password' --admin_email=admin@example.invalid --skip-email --allow-root
...
"${compose[@]}" cp "$root/tests/integration/issue52-comment-administration-smoke.php" wordpress:/var/www/html/wp-content/plugins/wp-native-builder-bridge/tests/integration/issue52-comment-administration-smoke.php
actual_wp="$("${wp[@]}" core version --allow-root | tail -n 1)"
actual_php="$("${wp[@]}" eval 'echo PHP_VERSION;' --allow-root | tail -n 1)"
echo "Issue #52 baseline: WordPress ${actual_wp}; PHP ${actual_php}; image ${wordpress_tag}"
"${wp[@]}" eval-file wp-content/plugins/wp-native-builder-bridge/tests/integration/issue52-comment-administration-smoke.php --user=1 --allow-root
echo 'Issue #52 comment administration integration: PASS'
