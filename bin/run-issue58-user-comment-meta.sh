...
export COMPOSE_PROJECT_NAME="wpai-issue58-${safe_tag}-$$"
...
"${wp[@]}" core install --url=https://localhost --title='WP AI Bridge Issue 58' --admin_user=admin --admin_password='integration-only-password' --admin_email=admin@example.invalid --skip-email --allow-root
...
for test_file in \
    ...
    "${compose[@]}" cp "$root/tests/integration/${test_file}" "wordpress:/var/www/html/wp-content/plugins/wp-ai-bridge/tests/integration/${test_file}"
done
...
"${wp[@]}" eval-file wp-content/plugins/wp-ai-bridge/tests/integration/issue58-user-comment-meta-smoke.php --user=1 --allow-root
"${wp[@]}" eval-file wp-content/plugins/wp-ai-bridge/tests/integration/issue58-user-comment-meta-policy-smoke.php --user=1 --allow-root
"${wp[@]}" eval-file wp-content/plugins/wp-ai-bridge/tests/integration/issue58-user-comment-meta-race-smoke.php --user=1 --allow-root
"${wp[@]}" eval-file wp-content/plugins/wp-ai-bridge/tests/integration/issue58-user-comment-meta-create-race-smoke.php --user=1 --allow-root
"${wp[@]}" eval-file wp-content/plugins/wp-ai-bridge/tests/integration/issue58-user-comment-meta-storage-smoke.php --user=1 --allow-root
