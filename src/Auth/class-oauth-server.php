...
final class OAuth_Server {
	...
	/** Registers canonical WordPress and MCP Adapter hooks. @return void */
	...
	/** Registers the canonical WP AI Bridge HTTP server. @param object $adapter MCP Adapter instance. @return void */
	...
	/** Registers OAuth token and revocation endpoints under the canonical namespace. @return void */
	...
	/** Serves canonical public metadata and browser authorization. @return void */
	...
	private function is_supported_resource_url( $resource_url ) {
		return hash_equals( $this->mcp_endpoint_url(), (string) $resource_url );
	}
	...
	private function request_resource_url( $request ) {
		return $request instanceof \WP_REST_Request && self::MCP_REQUEST_ROUTE === $request->get_route()
			? $this->mcp_endpoint_url()
			: '';
	}
	...
	/** @param \WP_REST_Request|null $request Optional MCP request. @return string WWW-Authenticate value. */
	private function www_authenticate_header( $request = null ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found -- Signature retained for filter call site.
		return sprintf( 'Bearer resource_metadata="%s", scope="%s"', $this->protected_resource_metadata_url(), implode( ' ', $this->supported_scopes() ) );
	}
