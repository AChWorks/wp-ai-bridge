...
WP AI Bridge is a small, free, self-hosted WordPress plugin whose goal is to make the full range of administration available to a real WordPress administrator discoverable and delegable to an AI through the Bridge. The canonical install identity is `wp-ai-bridge/wp-ai-bridge.php`. A bounded one-time migration release may read the former WP Native Builder Bridge identifiers only to migrate an existing installation; after cutover, runtime paths and new persisted state use the WP AI Bridge identity.
